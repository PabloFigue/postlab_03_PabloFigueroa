...
; configuration word 1
  CONFIG FOSC = INTRC_NOCLKOUT ; Oscillator Selection bits (INTOSC oscillator: CLKOUT function on ((PORTA) and 07Fh), 6/OSC2/CLKOUT pin, I/O function on ((PORTA) and 07Fh), 7/OSC1/CLKIN)
...
timer:
    ...
    btfss INTCON, 2 ; verificar si la bandera ((INTCON) and 07Fh), 2 esta en 1, se salta la siguiente linea, de lo contrario lee lo inmediato
...
config_io:
    ...
    bsf STATUS, 6
    clrf ANSEL ; 0 = pines digitales, ANS<4:0> = PORTA, ANS<7:5> = PORTE
...
config_reloj:
    ...
    bcf ((OSCCON) and 07Fh), 6 ; OSCCON, 6
    bsf ((OSCCON) and 07Fh), 5 ; OSCCON, 5
    bsf ((OSCCON) and 07Fh), 4 ; OSCCON, 4
